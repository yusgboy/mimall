  ...
  <div class="order-confirm">
    ...
    <template v-slot:body>
      <div class="edit-wrap">
        ...
        <div class="item">
          <select name="province" id="" v-model="checkedItem.receiverProvince" @change="updateCity();updateDistrict()">
            <option value="请选择省份" disabled>请选择省份</option>
            <option  v-for="(item,index) in provinceCityDistrict" :key="index">{{item.name}}</option>
          </select>
          <select name="city" id="" v-model="checkedItem.receiverCity" @change="updateDistrict">
            <option value="请选择市区" disabled>请选择市区</option>
          ...
          <select name="district" id="" v-model="checkedItem.receiverDistrict" >
            <option value="请选择区/县" disabled>请选择区/县</option>
            <option  v-for="(item,index) in districtArr" :key="index">{{item.name}}</option>
          </select>
        </div>
        <div class="item">
...
export default {
  data() {
    return {
      ...
      showDelModal:false,//是否显示弹窗
      showEditModal:false,//是否显示新增或编辑弹窗
      checkIndex:0,
  ...
  methods:{
    ...
    openAddressModal(){
      this.userAction=0;
      this.checkedItem={receiverProvince:'请选择省份',receiverCity:'请选择市区',receiverDistrict:'请选择区/县'};
      this.showEditModal=true;
    },
    editAddressModal(item){
      this.userAction=1;
      this.checkedItem =item;
      this.provinceCityDistrict.forEach((item)=>{
        if(item.name==this.checkedItem.receiverProvince){
          this.cityArr=item.children;
        }
      })
      this.cityArr.forEach(item=>{
        if(item.name==this.checkedItem.receiverCity){
    ...
    getProvinceCityDistrict(){//mock的地址信息
      this.$axios.get('/getProvinceCityDistrict').then((res)=>{
        this.provinceCityDistrict=res;
      })
    },
    updateCity(){
      this.provinceCityDistrict.forEach((item)=>{
      ...
      this.checkedItem.receiverCity=this.cityArr[0].name;
    },
    updateDistrict(){
      this.cityArr.forEach(item=>{
        if(item.name==this.checkedItem.receiverCity){
      ...
      this.checkedItem.receiverDistrict=this.districtArr[0].name;
    }
  },
  components: {
Modal,
